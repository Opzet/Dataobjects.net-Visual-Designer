...
'        Return If(propertyBase.Owner.TypeKind = PersistentTypeKind.[Interface], String.Empty, MODIFIER_VIRTUAL)
'    End Function
...
'    Public Function InheritanceList(persistentType As IPersistentType) As String()
...
'            result = New List(Of String)() From { _
'             OrmUtils.BuildXtensiveType(OrmType.EntitySet, EscapeNameWithNamespace(typedEntitySet.ItemType, persistentType)) _
'            }
'            Return result.ToArray()
'        End If

'        Dim [interface] As IInterface = TryCast(persistentType, IInterface)
'        Dim entityBaseBaseType As String = Nothing
...
'            End If
'            Dim allInheritanceList = inheritanceTree.GetFlatList(InheritanceListMode.CurrentLevel).[Select](Function(node) node.[Interface])
...
'                entityBaseBaseType = If(entityBase.BaseType Is Nothing, Nothing, EscapeNameWithNamespace(entityBase.BaseType, persistentType))
'            End If
'        End If

'        Dim commonBaseType As String = If(persistentType.TypeKind = PersistentTypeKind.[Structure], OrmUtils.BuildXtensiveType(OrmType.[Structure]), OrmUtils.BuildXtensiveType(OrmType.Entity))

'        Dim noInheritance As Boolean = result Is Nothing OrElse result.Count = 0 AndAlso persistentType.TypeKind <> PersistentTypeKind.[Interface]
...
'                If [interface].InheritingByInterfaces.Count = 0 AndAlso String.IsNullOrEmpty(entityBaseBaseType) Then
'                    result.Insert(0, commonBaseType)
'                End If
'            End If
'        End If

'        If Not String.IsNullOrEmpty(entityBaseBaseType) Then
'            result.Insert(0, entityBaseBaseType)
'        End If
...
'                        For Each attributeGroupItem As KeyValuePair(Of String, Defaultable) In attributeGroupItems
...
'                                Dim buildedCustomFormat As Boolean = attributesBuilder.BuildCustomFormat(attributeGroup, attrItemName, valueLiteral, attr)
